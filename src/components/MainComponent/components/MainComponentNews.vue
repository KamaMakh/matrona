<template>
  <div class="main-component-news">
        ...
        <v-col cols="12" sm="12" md="8" xs="12">
          ...
          <v-radio-group v-model="article.articlecoverType" :rules="rules">
            <v-row class="align-center">
              ...
              <v-col
                v-if="article.articleid && article.articlecoverType !== 'video'"
                cols="2"
                sm="5"
              >
                <viewer
    ...
    <v-form ref="form2" v-if="isNew" v-model="valid" lazy-validation>
      ...
      <v-card v-if="articleNew.createNotification" max-width="640">
        <v-card-text>
          <v-row>
            <v-col>
    ...
    <v-dialog v-model="deleteDialog" max-width="290">
      <v-card>
        <v-card-title class="headline"
          >Удалить статью <br />
          {{ article.articletitleShort }}?</v-card-title
        >
        <v-card-actions>
          <v-spacer></v-spacer>
        ...
        if (articleObj.hasOwnProperty(key)) {
          if (
            ...
            if (this.isNew) {
              ...
              formData.delete("article[sendNotification][type]");
              formData.delete("article[sendNotification][title]");
              formData.delete("article[sendNotification][body]");
              formData.delete("article[sendNotification][phones]");
              formData.delete("article[sendNotification]");
            }
          }
        }
      }
